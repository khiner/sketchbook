...
class Body extends Vec2D {
    ...
    Body() { super(0,0); } // the compiler needs this for default.  never used explicitly
    ...
    public void display() {
	if (animate) {
	    if (TICK - animateStart < 6)
		++displaySize;
	    else if (TICK - animateStart < 11)
		--displaySize;
	    else
		animate = false;
	}
    ...
    public void setSize(float sz) { displaySize = sz; }
	...
	switch(lNum) {
	case 0: return new Vec2D(ellipseCenter + ellipseA*sin(angleAtTime(steps)), ellipseB*cos(angleAtTime(steps)));
	case 1: return parent.locationAtTime(steps).scale(.95f);
	case 2: return parent.locationAtTime(steps).scale(1.15f);
	case 3: return parent.locationAtTime(steps).getRotated(PI);
	case 4: return parent.locationAtTime(steps).getRotated(PI/3);
	case 5: return parent.locationAtTime(steps).getRotated(-PI/3);
	default: return null;
	}
    }
...
class BodyGraph implements Iterable<Body>, Iterator<Body>
{
    ...
    public ArrayList<Body> vertices() { return (ArrayList)vertices; }
...
class Rocket extends Body
{
    ...
    public void setSize(float newSize) { displaySize = newSize; }
...
public void fin(float sz, float angle) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#DFDFDF", "Solar_Transport" });
  }
}
